...
class GainersAdapter(context: Context) : RecyclerView.Adapter<GainersAdapter.UserViewHolder>() {
    ...
    fun submitList(newcoins: List<Coin>) {
        val minus = newcoins.filter { it.price_change.startsWith("-") }
        val plus = newcoins.filter { !it.price_change.startsWith("-") }
        coins.clear()
        coins.addAll(plus)
        coins.addAll(minus)
